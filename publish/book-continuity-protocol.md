...
6. 任何章节如果发现把高版本内容写进低版本主线，必须先回修版本边界，再继续扩写。
...
7. 低版本章节不得直接展开高版本才出现的核心结构、协议或运行时能力。
8. 一个章节如果绑定多个版本，这些版本必须是连续区间，而不是离散拼接。
9. 章节正文必须显式体现上一章如何过渡到本章，以及本章如何引向下一章。
...
如果发现某章已经写入了错误版本边界内容，后续处理顺序必须固定为：

1. 先确认该章真正对应的主版本或连续版本区间
2. 标记哪些段落混入了后续高版本内容
3. 先修正文边界，再修代码入口与版本定位
4. 同步更新 `publish/chapter-map.md`、`publish/version-index.md`、修订记录和当前摘要
5. 边界未修正前，不继续做配图、排版或进一步扩写
